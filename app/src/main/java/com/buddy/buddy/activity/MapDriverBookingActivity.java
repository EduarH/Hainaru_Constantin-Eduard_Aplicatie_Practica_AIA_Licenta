...
import com.buddy.buddy.utils.DecodePoints;
...
public class MapDriverBookingActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private void startBooking() {
        clientBookingController.updateStatus(extraIdClient,"start").addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void unused) {
                ...
                map.clear();
                map.addMarker(new MarkerOptions().position(destinationLatLng).title("Destination").icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_person_destination)));
        ...
        clientBookingController.getClientBooking(extraIdClient).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    map.addMarker(new MarkerOptions().position(originLatLng).title("Pick up here").icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_person_location)));
        ...
        if (userController.existSession() && mlatLng != null){
            ...
            if (!isCloseClient){
                if (originLatLng != null && mlatLng != null){
                    double distance = getDistanceBetween(originLatLng,mlatLng); //meters
                    if (distance <= 200){
                        isCloseClient = true;
                        buttonNext.setVisibility(View.VISIBLE);
                        Toast.makeText(this, "You are close to the customer.", Toast.LENGTH_SHORT).show();
                    }
                }
            }
    ...
    private void showAlertDialogGPS(){
        ...
        builder.setTitle("GPS permission")
                .setMessage("GPS needs to be active.")
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED){
            if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)){
                new MaterialAlertDialogBuilder(this)
                        ...
                        .setMessage("This app requires location permissions to continue.")
    ...
    private void disconnect(){
        if (fusedLocationClient != null){
            ...
            Toast.makeText(this, "You can't disconnect", Toast.LENGTH_SHORT).show();
        }
    }
